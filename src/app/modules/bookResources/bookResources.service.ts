...
//Create Function
const createBookResources = async (payload:Prisma.BookResourcesCreateInput) => {
  const result = await prisma.bookResources.create({
      data: payload,
    });

  if (!result) {
    throw new AppError(httpStatus.BAD_REQUEST, 'Failed to create bookResources');
  }
  return result;
};
...
// update 
const updateBookResources = async (id: string, payload:Prisma.BookResourcesUpdateInput ) => {
 const result = await prisma.bookResources.update({
    ...
    if (!result) 
      throw new Error('Failed to update BookResources');
...
const deleteBookResources = async (id: string) => {
 ...
 const result = await prisma.bookResources.update({
  ...
  if (!result) 
    throw new AppError(httpStatus.BAD_REQUEST, 'Failed to delete bookResources');
